...
const int alturaIncremento = 8;  // cuánto sube por cada pieza
...
void rutina2() {
  ...
  int alturaActual = alturaBase + ((pilaCount - 1) * alturaIncremento );
  int baseActual = 170 - ((pilaCount - 1) * alturaIncremento);
...
void procesarComandoRutina() {
  ...
  // Detectar si el comando empieza con 'S'
  if (cmd.startsWith("S")) {
    cmd = cmd.substring(1);  // eliminar la S
    cmd.trim();
    servo = true;
  }
  ...
  int a = cmd.substring(0, coma).toInt();
  int b = cmd.substring(coma + 1).toInt();

  // Detectar si el comando empieza con 'S'
  if (servo) {
    switch (a) {
    case 1: servo1.write(b); pos1 = b; break;
    case 2: servo2.write(b); pos2 = b; break;
    case 3: servo3.write(b); pos3 = b; break;
    case 4: servo4.write(b); pos4 = b; break;
    }
    return;
  }
  ...
  if (a >= 1 && a <= 4 && b > 0 && b < 5) {
    for (int i = 0; i < b; i++) {
      switch(a) {
        ...
        case 3: rutina3(); break;
        case 4: rutina4(); break;
      }
      deteneRutina();
    }
    Serial.println("Rutina completada");
    return; 
  }
}
...
void deteneRutina() {
  ...
  if (cmd == "0,0") {
    ...
    servo4.write(pos4);
    delayNonBlocking(200);
    wdt_enable(WDTO_30MS);  // reset fuerte
