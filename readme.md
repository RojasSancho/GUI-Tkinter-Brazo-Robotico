# README

Utilice este comando en la terminal si desea crear un ejecutable del proyecto (.exe):
```
pyinstaller --clean --onefile --noconsole ventana.py
```

## Requisitos previos:
Antes de ejecutar la aplicación, asegúrese de instalar la fuente Bebas Neue que se encuentra en la carpeta fonts/ de este proyecto.
Esto garantiza que todos los textos y widgets se muestren correctamente.

Ademas, ejecute este comando en la terminal, estando dentro de la carpeta del proyecto:
```
pip install -r requirements.txt
```
Esto para garantizar que las dependencias se instalen correctamente, antes de ejecutar la aplicacion.
